...
from .anchor_mamba import RMSNorm, LayerScale2

def make_optimizer(model, opt):
    decay, no_decay = set(), set()
    whitelist_modules = (nn.Linear, nn.Conv1d, MaskedConv1D)
    blacklist_modules = (LayerNorm, Scale, LayerScale, RMSNorm, LayerScale2)
    ...
    for mn, m in model.named_modules():
        for pn, p in m.named_parameters():
            ...
            # Special handling for memory attention parameters (from EnhancedAnchorBlock_Memory)
                ...
                no_decay.add(fpn)
            elif pn.endswith('mean_query'):
                no_decay.add(fpn)
            elif '.scale' in pn:  # For LayerScale2 parameters
                no_decay.add(fpn)
            elif pn.endswith('bias'):
                no_decay.add(fpn)
            elif 'state_bridge_mlp' in pn:
                (no_decay if not pn.endswith('weight') else decay).add(fpn)
            elif 'moment_denoiser' in pn:
                (no_decay if not pn.endswith('weight') else decay).add(fpn)
            elif 'moment_rank_head' in pn:
                # Listwise ranking head: embeddings/bias no decay, linear
                # weights decay.
                (no_decay if not pn.endswith('weight') or
                 'level_embed' in pn else decay).add(fpn)
