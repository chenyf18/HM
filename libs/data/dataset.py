...
class BaseDataset(Dataset):
    ...
    def __init__(
        ...
        hcms_mode="off",        # HM-HCMS-001: off/random/hardneg/bg_control
        hcms_len_mode="strict", # strict | relaxed
        hcms_prob=0.0,
        ...
        self.hcms_mode = str(hcms_mode)
        self.hcms_len_mode = str(hcms_len_mode)
        self.hcms_prob = float(hcms_prob)
...
class VideoCentricDataset(BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.is_training:
            ...
            if self.hcms_mode != "off" and self.hcms_prob > 0:
                from .hcms import maybe_apply
                kwargs = {}
                if self.hcms_mode == "hard":
                    if not hasattr(self, "_hcms_pool"):
                        import json as _json
                        pool = {}
                        pool_file = os.path.join(
                            os.path.dirname(os.path.dirname(
                                os.path.dirname(os.path.abspath(__file__)))),
                            "experiments/hcms_pool/"
                            "hard_negative_pool.jsonl")
                        if os.path.exists(pool_file):
                            with open(pool_file) as f:
                                for line in f:
                                    r = _json.loads(line)
                                    pool[(r["vid_id"], r["query_idx"])] = \
                                        r["hard_negatives"]
                        self._hcms_pool = pool
                    gt_secs = vid_dict['segments'][
                        np.array(seg_idx)].tolist()
                    kwargs = dict(
                        query_pools=[
                            self._hcms_pool.get((vid_id, int(si)))
                            for si in seg_idx],
                        gt_secs=gt_secs,
                        fps=float(vid_dict['fps']),
                        clip_stride=float(self.clip_stride))
                vid_feats, segments, self._hcms_info = maybe_apply(
                    vid_feats, segments, self.hcms_mode,
                    len_mode=self.hcms_len_mode, prob=self.hcms_prob,
                    **kwargs)
            else:
                self._hcms_info = None
        ...
        return {
                 ...
                 'text_ids'   : tuple(vid_dict['text_ids'][idx] for idx in seg_idx),
                 'sentences'  : tuple(self.text_dict[text_id]['text'] for text_id in
                                      (vid_dict['text_ids'][idx] for idx in seg_idx)),
