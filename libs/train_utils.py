...
def generate_multiscale_gt_masks_from_points(fpn_points, targets):
    """Build point/span masks without assuming regular FPN spacing."""
    masks = []
    for points in fpn_points:
        if points.ndim == 2:
            points = points.unsqueeze(0).expand(targets.size(0), -1, -1)
        if points.ndim != 3 or points.size(0) != targets.size(0):
            raise ValueError("adaptive points must have shape (B, T, D)")
        if points.size(-1) >= 8:
            starts, ends = points[..., 4], points[..., 5]
        else:
            starts = points[..., 0] - 0.5 * points[..., 3]
            ends = points[..., 0] + 0.5 * points[..., 3]
        gt_start = torch.minimum(targets[:, 0], targets[:, 1])[:, None]
        gt_end = torch.maximum(targets[:, 0], targets[:, 1])[:, None]
        valid = points[..., 3] > 0
        masks.append((starts >= gt_start) & (ends <= gt_end) & valid)
...
def generate_multiscale_gt_masks_contrastive(points, targets, center_sampling_radius=1.5):
    ...
    for index, target in enumerate(targets):
        ...
        labels = annotate_points_per_video_span_aware(point_sample, target, center_sampling_radius)
        labels_list += (labels, )
...
def annotate_points_per_video_span_aware(points, target, center_sampling_radius):
    ...
    Returns:
        labels (bool tensor, (p,)): ground-truth binary labels.
    """
    # Adaptive points carry true support [start, end]; legacy points use
    # the fixed regression cell as their support.
    if points.size(-1) >= 8:
        span_starts = points[:, 4]
        span_ends = points[:, 5]
    else:
        span_starts = points[:, 0]
        span_ends = points[:, 0] + points[:, 3]
    ...
    labels = span_within_window & (points[:, 3] > 0)
